...
export default function EditMoviePage() {
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      try {
        const movie = await adminMovieService.getById(Number(movieId));
        setFormData({
          title: movie.title || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await adminMovieService.update(Number(movieId), {
        ...formData,
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-zinc-900">Chỉnh sửa phim</h1>
          <p className="text-zinc-500 mt-1">Cập nhật thông tin phim: {formData.title}</p>
        </div>
      </div>
